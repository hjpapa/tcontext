...
- 유치원·초등·중학교·고등학교 및 8개 역할별 질문 분기, 중고등학교 담당 교과 선택
...
원본 답변은 React 메모리에만 남으며 Web Storage, 쿠키, IndexedDB에 기록하지 않습니다. `sessionStorage`에는 학교급·역할·중고등학교 담당 교과·현재 질문 위치 같은 비민감 진행 메타데이터만 남고, 언제든 모든 브라우저 기록을 지울 수 있습니다. 새로고침하거나 탭을 닫으면 답변을 복구할 수 없습니다.
...
- 최종 `privacy_review`
- 학교급·역할·중고등학교 담당 교과 및 스키마·프롬프트·앱·동의·모델 버전
- 동의 시각, 보유 만료 시각
- 삭제 코드의 해시

원본 인터뷰의 전사본이나 별도 답변 필드, 이름, 이메일, IP 주소, 실제 학교명, 학생명, 브라우저 식별자, 삭제 코드 원문은 저장하지 않습니다. 교사가 근거별 문장과 AI 작성 제목·요약·원칙·지침을 검토한 최종 프로필만 선택적으로 저장하며, 이 프로필에는 학교급·학년·교과·대략적인 학급 규모·수업 환경·익명 집단 경향처럼 인터뷰에서 확인한 수업 맥락이 요약될 수 있습니다. RLS를 강제하고 `anon` 및 `authenticated`에는 읽기·쓰기 정책과 권한을 주지 않습니다.
