...
중학교와 고등학교 사용자가 선택한 `teachingSubject`는 통제된 열거값으로
`profile_json.metadata`에만 포함한다. 자유 입력 교과명이나 학교·학급명은 받지
않는다. 기존 프로필과의 호환을 위해 필드는 선택 사항이며, 새 문서는 담당 교과를
선택하지 않은 경우에도 명시적인 해당 없음 값을 사용할 수 있다.
